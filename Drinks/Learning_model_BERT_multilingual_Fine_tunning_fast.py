...
class ProductDataset(Dataset):
    def __init__(self, texts, labels, tokenizer, max_len, pretokenized_encodings=None):
        self.texts = texts
        self.labels = labels
        self.tokenizer = tokenizer
        self.max_len = max_len
        # Si se proveen codificaciones pre-tokenizadas, se usan directamente
        self.pretokenized_encodings = pretokenized_encodings

    def __len__(self):
        return len(self.texts)

    def __getitem__(self, idx):
        if self.pretokenized_encodings:
            encoding = {key: torch.tensor(val[idx]) for key, val in self.pretokenized_encodings.items()}
        else:
...
def create_data_loader(df, tokenizer, max_len, batch_size, shuffle=True):
    ...
    # Pre-tokenización para acelerar las iteraciones
    ...
    ds = ProductDataset(texts, labels, tokenizer, max_len, pretokenized_encodings=encodings)
...
class ProductDatasetForPrediction(Dataset):
    def __init__(self, texts, tokenizer, max_len, pretokenized_encodings=None):
        self.texts = texts
        self.tokenizer = tokenizer
        self.max_len = max_len
        self.pretokenized_encodings = pretokenized_encodings
...
def create_data_loader_for_prediction(df, tokenizer, max_len, batch_size):
    ...
    ds = ProductDatasetForPrediction(texts, tokenizer, max_len, pretokenized_encodings=encodings)

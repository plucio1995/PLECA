...
# Leer predicciones
# -----------------------------
file_path = "/Users/pedro.lucioglovoapp.com/PycharmProjects/PLECA/Drinks/predicciones_productos_GE_4_MultilingualBERT.csv"
...
mask_coca = (
    df['product_name'].str.contains("კოკა", case=False, na=False) |
    df['product_name'].str.contains("კოლა", case=False, na=False)
)

mask_pepsi = (
    df['product_name'].str.contains("პეპსი", case=False, na=False)
)

mask_sprite = (
    df['product_name'].str.contains("სპრაიტი", case=False, na=False)
)

mask_fanta = (
    df['product_name'].str.contains("ფანტა", case=False, na=False)
)

mask_agua = (
    df['product_name'].str.contains("წყალი", case=False, na=False) |       # "cqali"
    df['product_name'].str.contains("მთისწყალი", case=False, na=False) |  # "mt'iscqali"
    df['product_name'].str.contains("ბაკურიანისწყალი", case=False, na=False) |  # "cqalibakuriani"
    df['product_name'].str.contains("მინერალური", case=False, na=False)     # "mineraluri"
)

mask_kompot = (
    df['product_name'].str.contains("კომპოტი", case=False, na=False)
)

mask_mirinda = (
    df['product_name'].str.contains("მირინდა", case=False, na=False)
)

mask_redbull = (
    df['product_name'].str.contains("რედბული", case=False, na=False)
)

mask_lemonade = (
    df['product_name'].str.contains("ლიმონათი", case=False, na=False)
)
...
# -----------------------------
df.to_csv("test products_GE_6.csv", index=False)
